...
class TestSessionStore:
    def test_create_new_session(self):
        store = SessionStore()
        session = store.get_messages("abc-123")
        assert session == []

    def test_get_existing_session(self):
        store = SessionStore()
        store.get_messages("abc-123")
    ...
    def test_append_to_nonexistent_session_creates_it(self):
        ...
        session = store.get_messages("new-id")
        assert len(session) == 1
    ...
    def test_sessions_are_isolated(self):
        ...
        assert len(store.get_messages("session-1")) == 1
        assert len(store.get_messages("session-2")) == 1
        assert store.get_messages("session-1")[0].content == "one"
